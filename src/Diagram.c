...
Diagram Diagram_Make(const struct LogBook *logBook, size_t currentLogIndex) {
  ...
  for (LogEntry *log = logBegin; log < logEnd; ++log) {
    ...
    case EOperation_Remove: {
      ...
      if (node == NULL) {
        fprintf(stderr, "Unable to find node %s at log %ld",
                LogBook_GetNodeName(logBook, log->nodeName), log->timestamp);
        exit(10);
      }
...
static Node *Diagram_FindNode(Diagram const *diagram, size_t nodeName) {
  for (Node *it = diagram->nodes; it < diagram->nodes + diagram->nodesSize;
       ++it) {
